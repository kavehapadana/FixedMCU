...
int main(void)
{
  ...
  Init_Timer0();
	...
	GPIOINT_Init(&proxyPin, 3, 15, EXTI_POLARITY_HIGH_ACTIVE_OR_RISING_EDGE);
...
void Serials_Init(void)
{
	Com_Init(UART_RF,ENABLE,DISABLE,26,UART_RF_BAUDRATE,1);
  Com_Init(UART_PC,ENABLE,DISABLE,10,UART_PC_BAUDRATE,1);
	Com_Init(UART_AC_Motor,ENABLE,DISABLE,14,UART_AC_Motor_BAUDRATE,1); 	
}
...
{
	if(Message_ID == msgTrans_R_Detect_Reset_ID)		
	{
			UARTSend(UART_RF, Make_Trans_Msg(msgTrans_R_Detect_Reset_ID,msg,1) , (TRANS_HEADER_NO_PC+1));
	}
...
void Init_Timer0(void)
{
	...
	// Set Match value
	TIM_MatchConfigStruct.MatchValue   = 2;
	// Set configuration for Tim_MatchConfig
	TIM_ConfigMatch(LPC_TIM0,&TIM_MatchConfigStruct);
		/* preemption = 1, sub-priority = 1 */
	NVIC_SetPriority(TIMER0_IRQn, ((0x01<<8)|0x01));
...
void Init_Timer1(void)
{
	...
	TIM_MatchConfigStruct.ExtMatchOutputType = TIM_EXTMATCH_NOTHING;
	// Set Match value
	TIM_MatchConfigStruct.MatchValue   = 5 - 1; // x - 1 : x is matchvalue // 4 KHz / 5 = 800 Hz
	// Set configuration for Tim_MatchConfig
